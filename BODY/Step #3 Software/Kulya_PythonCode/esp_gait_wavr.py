#this is gait - wavr - wave right leg
#it takes:
#   collection of legs
    ...
    if frame == 1:
        for leg in legs:
            leg.target_angles = IK(leg, getSpread_xyz(leg,
                                                      ...
                                                      ry,
                                                      rz))
        legs[5].target_angles = [0,60,8]

                 
    if frame == 2:
        esp_context.motion_smooth_rate = .0001
        legs[5].target_angles = [-20,60,8]
...
def gait_wavr(legs):
    ...
    print("gait:wavr - wave right leg")



    while esp_context.RC_data["gait"] == 'wavr':   #until recieved new gait, keep doing current walk routine
